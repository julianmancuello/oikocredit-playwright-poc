...
Then("all steps test", async () => {
  ...
  await pm.onLegalDetailsPageOMOC().selectCountryOfTaxResidency("Germany")
  await pm.onLegalDetailsPageOMOC().isTaxExempt(false)
  await pm.onLegalDetailsPageOMOC().fillTaxIdentificationNumber()
  await pm.onLegalDetailsPageOMOC().isAmericanCitizen(false)
  await pm.onLegalDetailsPageOMOC().checkMandatoryCheckboxes(true)
  //await pm.onLegalDetailsPageOMOC().footerOMOC.clickOn("Next")
})